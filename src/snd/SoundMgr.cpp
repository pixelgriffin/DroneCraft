...
void SoundMgr::initialize(void){
	...
	//syncListenerToCamera(); //setup listener
        ...
        //initialize vectors
        for (int i = 0; i < FastEcslent::NENTITYTYPES; i++){
...
bool SoundMgr::initWatercraftSounds(){
        ...
        for (int i = 0; i < FastEcslent::NENTITYTYPES; i++){
            FastEcslent::EntityType et = static_cast<FastEcslent::EntityType>(i);
            //this->registerBattleSound(et, battleFilename);
            if (isEntityShip(et)){
                if (et == FastEcslent::MARINE){ //marine has 2 different selection sounds and chooses randomly. just to illustrate.
...
void SoundMgr::syncListenerToCamera(){
	...
	Ogre::Vector3 cameraPosition = gim->uiMgr->cameraMgr->cameraNode->getPosition();
	...
	Ogre::Vector3 cameraVelocity = gim->uiMgr->cameraMgr->velocity; // not set by graphics
	this->velocity[0] = 0;
	this->velocity[1] = 0;
	this->velocity[2] = 0;
	...
	Ogre::Quaternion q = gim->uiMgr->cameraMgr->cameraNode->getOrientation();
...
void SoundMgr::attachSelectedNodeToSoundIndex(OgreGFX::GFXNode *gfxNode, unsigned int index){
...
void SoundMgr::tick(double dtime){
	...
	syncListenerToCamera();
        ...
        for(int i = 0; i < gim->nGFXNodes; i++){
            FastEcslent::Entity *ent = gim->engine->entityMgr->ents[i];
            //only play for selection of current player's entities
            if (ent->engine->options.player != ent->entityId.player){
                        continue;
            }
            else {
                        FastEcslent::EntityType et = ent->entityType;
                        if (ent->isSelected && !ent->didSelectSoundPlayed){
                                playSelectionSound(et, &(gim->gfxNodes[i]));
                                ent->didSelectSoundPlayed = true;
                        }
                        else if (!ent->isSelected && ent->didSelectSoundPlayed){
                                ent->didSelectSoundPlayed = false;
                            }
            }
	}
        
        //new born or new built sound
        for(int i = 0; i < gim->nGFXNodes; i++){
            FastEcslent::Entity *ent = gim->engine->entityMgr->ents[i];
            //only play for creation of entities belonging to current player
            if (ent->engine->options.player != ent->entityId.player){
                        continue;
            }
            else {
                        FastEcslent::EntityType et = ent->entityType;
                        if (ent->isNewBorn){
                                playEntityBornSound(et, &(gim->gfxNodes[i]));
                                ent->isNewBorn = false;
                        }
            }
	}
...
bool SoundMgr::playSelectionSound(FastEcslent::EntityType et, OgreGFX::GFXNode *gfxNode){
        Ogre::Vector3 pos = gfxNode->node->getPosition();
        
        int sounds = 0;
        int arrayIndex = 0;
        for (int i = 0; i < soundPerEnt; i++){
            if (selectionSoundsDictionary[et][i] != -1)
                sounds++;
            else
                break;
        }
        ...
        int sourceIndex = selectionSoundsDictionary[et][arrayIndex];
        this->playAudioSourceIndex(sourceIndex);
...
bool SoundMgr::registerSelection(FastEcslent::EntityType et, std::string filename){
    unsigned int sid;
    //check if that file is already assigned to a source
    for (int j = 0; j < maxAudioSources; j++){
        if (std::strcmp(sourceDictionary[j].c_str(), filename.c_str()) == 0){
                int lastIndex = -1;
                for (int i = 0; i < soundPerEnt; i++){
                    if (this->selectionSoundsDictionary[et][i] == -1){
                        lastIndex = i;
                        break;
                    }
                }
                ...
                this->selectionSoundsDictionary[et][lastIndex] = j;
                return true;
        }
    }
    if (this->reserveAudio(filename, false, sid)){
                ...
                this->selectionSoundsDictionary[et][lastIndex] = sid;
                alSourcei(this->sourceInfo[sid].source, AL_REFERENCE_DISTANCE, 2000.0f);
                alSourcei(this->sourceInfo[sid].source, AL_MAX_DISTANCE, 8000.0f);
...
bool SoundMgr::loadAudio(std::string filename, int index){
	...
	this->bufferInfo[index].wave = WaveOpenFileForReading(fqfn.c_str());
